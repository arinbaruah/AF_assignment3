...
pop_fc %>% accuracy(pop_test) %>% arrange(RMSE) %>% 
  select(-c(ME,MAE,MPE,MAPE,MASE,RMSSE)) %>% kbl()
```
...
```
As we can observe from @fig-model, __the mean and the naïve models have completely missed the overall trend of the logarithmic population rise__ while the __drift and the ETS models appear to follow the overall global trend very closely, albeit we can observe that the drift model has a narrower confidence interval compared to the ETS model.__
...
pop_train %>% stretch_tsibble(.init = 15,.step = 1) %>% 
                    ...
                    mean = MEAN(Log_population), # Mean model
                    drift = RW(Log_population ~ drift())) %>% # Drift model
                             forecast(h = "5 years") %>% accuracy(pop_test) %>% arrange(RMSE) %>% select(-c(ME,MAE,MPE,MAPE,MASE,RMSSE)) %>% kbl()
...
After performing cross-validation of the data and obtaining the model metrics as illustrated by @tbl-cv, __we observe that the drift model remains the one that performs the best followed by the ETS model.__ 

This indicates that __even with the implementation of the cross-validation, the drift model is observed to train itself better due to the availability of more data and outperform the ETS model similar to what was observed in @sec-initial .__ 
